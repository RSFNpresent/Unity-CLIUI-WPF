...
1. 只有设置中明确选择 `Direct`（无 CLI）模式时，才允许手动下载并安装 Unity 官方包。
2. `Auto` 只检测已有 Unity CLI，不得隐式切换为直接下载；`UnityCli` 只走 CLI 后端。
3. Direct 模式的编辑器版本详情只显示本机已安装模块；远程模块目录不可达时，其他模式也回退到已安装模块，不使用全量缓存冒充实时结果。
4. 当前版本通过集中策略跟随 Windows 动画设置；页面、亚克力和窗口状态动画不得阻塞最终状态切换。
5. 编辑器与模块数据只能来自 Unity 官方 Release API 和官方分发地址，并执行完整性、路径与包类型校验。
6. 永久拒绝 `System.Security.Cryptography.Xml.dll`；它是测试文件，不属于官方分发内容。
7. 发布 ZIP 必须保持单 `Unity-CLIUI.exe` 布局，不包含 Unity CLI，也不附带额外 DLL。
8. 创建工程不使用官方模板，只生成最小必要目录和文件；包版本必须来自所选编辑器官方元数据。
9. 创建成功后立即加入工程列表，并使用用户选择的准确编辑器启动。
10. 工程测试目录使用 `E:\`，不得在 `D:\` 创建 Unity 测试工程。
11. 不得终止用户正在运行的 `E:\Unity-CLIUI.exe` 或 Unity 编辑器进程。
